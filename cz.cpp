...
int main()
{
 ...
 gen.close();
 	 ofstream shuchu("D:\shuchu.xlsx");
    if (!shuchu.is_open())
    {
        cout<<"open error";
    }
    while(!shuchu.eof() && n<7)
	{
			gen >> judge[n].name >>judge[n].score[0]>>judge[n].score[1]>>judge[n].score[2]>>judge[n].score[3]>>judge[n].score[4];
		    n++;
	 } 
 ...
 int i=0,j=0;
	...
	for(int i=1;i<6;i++)
	{
		shuchu<<student[i].college<<" "<<student[i].major<<" "<<student[i].number<<" "<<student[i].name<<" "
		<<student[i].sex<<" "<<student[i].sum<<endl;
	} 
	for(int i=1;i<6;i++)
	{
		cout<<student[i].college<<" "<<student[i].major<<" "<<student[i].number<<" "<<student[i].name<<" "
		<<student[i].sex<<" "<<student[i].sum<<endl;
	} 
		cout<<" err ";
return 0;
